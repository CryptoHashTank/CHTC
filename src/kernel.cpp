...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZChtcStake(spend));
    } else {
        ...
        CChtcStake* chtcInput = new CChtcStake();
        chtcInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(chtcInput);
    }
