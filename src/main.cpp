...
#include "zchtc/accumulatormap.h"
#include "addrman.h"
#include "alert.h"
...
#if defined(NDEBUG)
#error "CHTC cannot be compiled without assertions."
#endif
...
unsigned int nStakeMinAge2 = 12 * 60 * 60;
int64_t nReserveBalance = 0;

/** Fees smaller than this (in uchtc) are considered zero fee (for relaying and mining)
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock)
{
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zCHTC spend with serial %s is already in block %d\n", __func__,
...
{
    //Check to see if the zCHTC is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        try {
            if (!spend.HasValidSignature())
                return error("%s: V2 zCHTC spend does not have a valid signature\n", __func__);
        ...
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zCHTC without the correct spend type. txid=%s\n", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (!spend.HasValidSerial(Params().Zerocoin_Params(fUseV1Params))) {
        ...
        if (!isBlockBetweenFakeSerialAttackRange(pindex->nHeight))
            return error("%s : zCHTC spend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zCHTC spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                               tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zchtc");
            }
        } else {
            LOCK(pool.cs);
            ...
            // Check that zCHTC mints are not already known
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZCHTCStake)
{
    int64_t ret = 0;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from CHTC
...
void RecalculateZCHTCMinted()
{
    CBlockIndex *pindex = chainActive[Params().Zerocoin_StartHeight()];
    uiInterface.ShowProgress(_("Recalculating minted ZCHTC..."), 0);
    while (true) {
        ...
        if (pindex->nHeight % 1000 == 0) {
            ...
            uiInterface.ShowProgress(_("Recalculating minted ZCHTC..."), percent);
        }
...
void RecalculateZCHTCSpent()
{
    CBlockIndex* pindex = chainActive[Params().Zerocoin_StartHeight()];
    uiInterface.ShowProgress(_("Recalculating spent ZCHTC..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = std::max(1, std::min(99, (int)((double)(pindex->nHeight - Params().Zerocoin_StartHeight()) * 100 / (chainActive.Height() - Params().Zerocoin_StartHeight()))));
            uiInterface.ShowProgress(_("Recalculating spent ZCHTC..."), percent);
        }

        //Rewrite zCHTC supply
        CBlock block;
        assert(ReadBlockFromDisk(block, pindex));
        ...
        //Remove spends from zCHTC supply
...
bool RecalculateCHTCSupply(int nHeightStart)
{
    ...
    uiInterface.ShowProgress(_("Recalculating CHTC supply..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = std::max(1, std::min(99, (int)((double)((pindex->nHeight - nHeightStart) * 100) / (chainActive.Height() - nHeightStart))));
            uiInterface.ShowProgress(_("Recalculating CHTC supply..."), percent);
        }
...
bool UpdateZCHTCSupply(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    std::list<CZerocoinMint> listMints;
    ...
    // A one-time event where only the zCHTC supply was off (due to serial duplication off-chain on main net)
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateCHTCSupply(Params().Zerocoin_StartHeight());
    }

    //Track zCHTC money supply in the block index
    if (!UpdateZCHTCSupply(block, pindex, fJustCheck))
        return state.DoS(100, error("%s: Failed to calculate new zCHTC supply for block=%s height=%d", __func__,
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zChtcSpent: %s\n",
...
#ifdef ENABLE_WALLET
    // If turned on AutoZeromint will automatically convert CHTC to zCHTC
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zCHTC spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn& txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zCHTC serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zCHTC stake block is less than allowed start height", __func__);

    if (CZChtcStake* zCHTC = dynamic_cast<CZChtcStake*>(stake)) {
        CBlockIndex* pindexFrom = zCHTC->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zCHTC stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zCHTC stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, chainActive.Height(), pindexFrom->nHeight);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zCHTC->GetValue()));
        if (nChecksum200 != zCHTC->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zCHTC->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZCHTC() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zCHTC fails context checks", __func__));
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> chtcInputs;
        std::vector<CTxIn> zCHTCInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if(stakeIn.scriptSig.IsZerocoinSpend()){
                zCHTCInputs.push_back(stakeIn);
            }else{
                chtcInputs.push_back(stakeIn);
            }
        }
        const bool hasCHTCInputs = !chtcInputs.empty();
        const bool hasZCHTCInputs = !zCHTCInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasCHTCInputs)
                    ...
                    for (const CTxIn& chtcIn : chtcInputs){
                        if(chtcIn.prevout == in.prevout){
        ...
        if (isBlockFromFork) {
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction &t : bl.vtx) {
                    for (const CTxIn &in: t.vin) {
                        ...
                        for (const CTxIn &stakeIn : chtcInputs) {
            ...
            // Now that this loop if completed. Check if we have zCHTC inputs.
            if(hasZCHTCInputs){
                for (const CTxIn& zChtcInput : zCHTCInputs) {
                    CoinSpend spend = TxInToZerocoinSpend(zChtcInput);
                    ...
                    if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, spend, pindex, 0))
                        return state.DoS(100,error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                                   stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zchtc");
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zCHTC mints and %d zCHTC spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // CHTC: We use certain sporks during IBD, so check to see if they are
        ...
        if(nLocalServices & NODE_BLOOM_LIGHT_ZC) {
            try {
                ...
                if (!GetAccumulatorValue(height, den, bnAccValue)) {
                    LogPrint("zchtc", "peer misbehaving for request an invalid acc checkpoint \n", __func__);
                    Misbehaving(pfrom->GetId(), 50);
                } else {
            ...
            try {
                ...
                if (gen.isValid(chainActive.Height())) {
                    if (!lightWorker.addWitWork(gen)) {
                        LogPrint("zchtc", "%s : add genwit request failed \n", __func__);
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
