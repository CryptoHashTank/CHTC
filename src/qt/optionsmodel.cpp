...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::CHTC);
    ...
    if (!settings.contains("nAnonymizeChtcAmount"))
        settings.setValue("nAnonymizeChtcAmount", 1000);

    nAnonymizeChtcAmount = settings.value("nAnonymizeChtcAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeChtcAmount"))
        SoftSetArg("-anonymizechtcamount", settings.value("nAnonymizeChtcAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in chtc.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeChtcAmount:
            return QVariant(nAnonymizeChtcAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case HideOrphans:
            ...
            break;
        case AnonymizeChtcAmount:
            nAnonymizeChtcAmount = value.toInt();
            settings.setValue("nAnonymizeChtcAmount", nAnonymizeChtcAmount);
            emit anonymizeChtcAmountChanged(nAnonymizeChtcAmount);
            break;
        case CoinControlFeatures:
