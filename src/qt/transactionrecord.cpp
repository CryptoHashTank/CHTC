...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.IsZerocoinSpend() && (fZSpendFromMe || wallet->zchtcTracker->HasMintTx(hash))) {
            //zCHTC stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZCHTC;
        ...
        for (const CTxOut& txout : wtx.vout) {
            ...
            if (txout.IsZerocoinMint()) {
                ...
                TransactionRecord sub(hash, nTime);
                sub.involvesWatchAddress = mine & ISMINE_WATCH_ONLY;
                sub.type = TransactionRecord::ZerocoinSpend_Change_zChtc;
                sub.address = mapValue["zerocoinmint"];
                if (!fFeeAssigned) {
...
bool IsZCHTCType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZCHTC:
        case TransactionRecord::ZerocoinMint:
        case TransactionRecord::ZerocoinSpend:
        case TransactionRecord::RecvFromZerocoinSpend:
        case TransactionRecord::ZerocoinSpend_Change_zChtc:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZCHTC || type == TransactionRecord::MNReward) {
