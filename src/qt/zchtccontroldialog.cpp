...
#include "ui_zchtccontroldialog.h"
...
std::set<std::string> ZChtcControlDialog::setSelectedMints;
std::set<CMintMeta> ZChtcControlDialog::setMints;

bool CZChtcControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZChtcControlDialog::COLUMN_DENOMINATION || column == ZChtcControlDialog::COLUMN_VERSION || column == ZChtcControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZChtcControlDialog::ZChtcControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZChtcControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZChtcControlDialog::~ZChtcControlDialog()
{
    delete ui;
}

void ZChtcControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZChtcControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZChtcControlWidgetItem* itemDenom(new CZChtcControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZChtcControlWidgetItem *itemMint = new CZChtcControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zchtcTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zchtcTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zCHTC.";
            else if (!mint.isSeedCorrect)
                strReason = "The zCHTC seed used to mint this zCHTC is not the same as currently hold in the wallet";
            else
...
void ZChtcControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZChtcControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZChtc_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZChtcControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZChtcControlDialog::GetSelectedMints()
{
...
void ZChtcControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
