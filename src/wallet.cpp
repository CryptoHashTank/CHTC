...
bool CWallet::SelectStakeCoins(std::list<std::unique_ptr<CStakeInput> >& listInputs, CAmount nTargetAmount)
{
    ...
    //zCHTC
    if (GetBoolArg("-zchtcstake", true) && chainActive.Height() > Params().Zerocoin_Block_V2_Start() && !IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
                         ...
                         CZerocoinSpendReceipt& receipt, libzerocoin::SpendType spendType, CBlockIndex* pindexCheckpoint)
{
    ...
    if (!pubCoinSelected.validate()) {
        receipt.SetStatus(_("The selected mint coin is an invalid coin"), ZCHTC_INVALID_COIN);
    ...
    if (!GenerateAccumulatorWitness(pubCoinSelected, accumulator, witness, nSecurityLevel, nMintsAdded, strFailReason, pindexCheckpoint)) {
        receipt.SetStatus(_("Try to spend with a higher security level to include more coins"), ZCHTC_FAILED_ACCUMULATOR_INITIALIZATION);
        return error("%s : %s", __func__, receipt.GetStatusMessage());
    }
    ...
    try {
        ...
        if (!spend.Verify(accumulator)) {
            receipt.SetStatus(_("The new spend coin transaction did not verify"), ZCHTC_INVALID_WITNESS);
            //return false;
        ...
        //Add the coin spend into a CHTC transaction
        ...
        try {
            ...
            receipt.SetStatus(_("Failed to deserialize"), ZCHTC_BAD_SERIALIZATION);
            return error("%s : %s", __func__, receipt.GetStatusMessage());
        }
    ...
    for (const auto& mint : vSelectedMints) {
        if (mint.GetVersion() < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
            if (nSecurityLevel < 100) {
                nStatus = ZCHTC_SPEND_V1_SEC_LEVEL;
                receipt.SetStatus(_("Version 1 zCHTC require a security level of 100 to successfully spend."), nStatus);
                return false;
            }
        }
    }
