...
extern bool fGlobalUnlockSpendCache; // Bool used for letting the precomputing thread know that zchtcspends need to use the cs_spendcache
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 CHTC at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zCHTC send
enum ZerocoinSpendStatus {
    ZCHTC_SPEND_OKAY = 0,                            // No error
    ZCHTC_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZCHTC_WALLET_LOCKED = 2,                         // Wallet was locked
    ZCHTC_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZCHTC_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZCHTC_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZCHTC_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZCHTC_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZCHTC_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZCHTC_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZCHTC_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZCHTC_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZCHTC_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZCHTC_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZCHTC_SPENT_USED_ZCHTC = 14,                      // Coin has already been spend
    ZCHTC_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZCHTC_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZChtcBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZCHTCOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzchtcAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzCHTCTracker> zchtcTracker;
    ...
    void setZWallet(CzCHTCWallet* zwallet)
    {
        zwalletMain = zwallet;
        zchtcTracker = std::unique_ptr<CzCHTCTracker>(new CzCHTCTracker(strWalletFile));
    }

    CzCHTCWallet* getZWallet() { return zwalletMain; }
    ...
    void setZChtcAutoBackups(bool fEnabled)
    {
    ...
    /** zCHTC reset */
    boost::signals2::signal<void()> NotifyzCHTCReset;
