...
public:
    ...
    bool WriteZCHTCSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZCHTCSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZCHTCSeed_deprecated(uint256& seed);
    bool EraseZCHTCSeed();
    bool EraseZCHTCSeed_deprecated();

    bool WriteZCHTCCount(const uint32_t& nCount);
    bool ReadZCHTCCount(uint32_t& nCount);
