...
CzCHTCWallet::CzCHTCWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zchtc seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZCHTCSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZCHTCSeed_deprecated()) {
                    LogPrintf("%s: Updated zCHTC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zchtc seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zchtc wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzCHTCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZCHTCCount(nCountLastUsed);
    else if (!walletdb.ReadZCHTCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzCHTCWallet::Lock()
{
    seedMaster = 0;
}

void CzCHTCWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzCHTCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZCHTC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzCHTCWallet::LoadMintPoolFromDB()
{
...
void CzCHTCWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzCHTCWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzCHTCWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zchtcTracker->HasPubcoinHash(pMint.first)) {
...
bool CzCHTCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZCHTC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zchtcTracker which also adds to database
    pwalletMain->zchtcTracker->Add(dMint, true);
...
void CzCHTCWallet::SeedToZCHTC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzCHTCWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzCHTCWallet::GenerateDeterministicZCHTC(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzCHTCWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzCHTCWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzCHTCWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
