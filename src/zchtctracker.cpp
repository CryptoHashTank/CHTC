...
CzCHTCTracker::CzCHTCTracker(std::string strWalletFile)
{
...
CzCHTCTracker::~CzCHTCTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzCHTCTracker::Init()
{
...
bool CzCHTCTracker::Archive(CMintMeta& meta)
{
...
bool CzCHTCTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzCHTCTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzCHTCTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzCHTCTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzCHTCTracker::GetSerialHashes()
{
...
CAmount CzCHTCTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzCHTCTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzCHTCTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzCHTCTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzCHTCTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzCHTCTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzCHTCTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzCHTCTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzCHTCTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzCHTCTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzCHTCTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzCHTCWallet* zCHTCWallet)
{
    bool iszCHTCWalletInitialized = (NULL != zCHTCWallet);
    CMintMeta meta;
    ...
    if (! iszCHTCWalletInitialized)
        zCHTCWallet = new CzCHTCWallet(strWalletFile);
    meta.isSeedCorrect = zCHTCWallet->CheckSeed(dMint);
    if (! iszCHTCWalletInitialized)
        delete zCHTCWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzCHTCTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzCHTCTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzCHTCTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzCHTCTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzCHTCTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzCHTCTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzCHTCWallet* zCHTCWallet = new CzCHTCWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            Add(dMint, false, false, zCHTCWallet);
        }
        delete zCHTCWallet;
        LogPrint("zero", "%s: added %d dzchtc from DB\n", __func__, listDeterministicDB.size());
    }
